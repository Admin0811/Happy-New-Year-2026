...
function typeMessage(message, i = 0) {
  if (i < message.length) {
    ...
    setTimeout(() => typeMessage(message, i + 1), 100); // tốc độ gõ chữ
  } else {
    // sau khi gõ xong 1 lời chúc
    setTimeout(() => {
      if (messageIndex < messages.length - 1) {
        // nếu chưa phải lời chúc cuối → xóa chữ cũ, gõ tiếp
        messageElement.textContent = "";
        messageIndex++;
        typeMessage(messages[messageIndex]);
      } else {
        // nếu là lời chúc cuối → giữ 2s, xóa rồi hiện modal
        setTimeout(() => {
          messageElement.textContent = "";
          document.getElementById("final-modal").classList.add("show");
        }, 2000); // thời gian giữ lời chúc cuối
      }
    }, 2000); // thời gian giữ mỗi lời chúc
  }
}

// Bắt đầu sau 5s (cho pháo hoa chạy trước)
setTimeout(() => {
  typeMessage(messages[messageIndex]);
}, 5000);
